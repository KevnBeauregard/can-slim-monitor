...
def stock_detail(ticker):
    import yfinance as yf
    from core.analysis import get_score

    info = yf.Ticker(ticker).info
    score = get_score(ticker)

    graph_url = f"https://quickchart.io/chart?c={{type:'sparkline',data:{{datasets:[{{data:[{','.join(get_sparkline(ticker))}]}}]}}}}"

    return render_template("stock.html", ticker=ticker.upper(), info=info, score=score, graph_url=graph_url)
